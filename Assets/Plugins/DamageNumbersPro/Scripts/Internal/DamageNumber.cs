    ...
    public abstract class DamageNumber : MonoBehaviour
    {
        ...
        void TryCollision(Vector3 sourcePosition)
        {
            if (enableCollision)
            {
                ...
                foreach (DamageNumber otherNumber in spamGroupDictionary[spamGroup])
                {
                    if (otherNumber.enableCollision && otherNumber.collisionImmuneTime != Time.unscaledTime)
                    {
        ...
        void TryPush(Vector3 sourcePosition)
        {
            if (enablePush)
            {
                pushImmuneTime = Time.unscaledTime;
                ...
                foreach (DamageNumber otherNumber in spamGroupDictionary[spamGroup])
                {
                    if (otherNumber.pushImmuneTime != Time.unscaledTime && otherNumber.enablePush)
                    {
